...
public class UserFormPage
{
    ...
    public UserFormPage(IPage page, string baseUrl)
    {
        _page = page;
        _baseUrl = baseUrl;
    }
    ...
    public async Task FillNameAsync(string name) => await NameInput.FillWithDelayAsync(name);
    public async Task FillEmailAsync(string email) => await EmailInput.FillWithDelayAsync(email);
    public async Task FillAgeAsync(string age) => await AgeInput.FillWithDelayAsync(age);
    public async Task FillPhoneAsync(string phone) => await PhoneInput.FillWithDelayAsync(phone);
    public async Task FillAddressAsync(string address) => await AddressInput.FillWithDelayAsync(address);
    public async Task SelectCountryAsync(string country) => await CountrySelect.SelectWithDelayAsync(country);
    public async Task FillBirthDateAsync(string date) => await BirthDateInput.FillWithDelayAsync(date);
    ...
    public async Task FillCompanyAsync(string company) => await CompanyInput.FillWithDelayAsync(company);
    public async Task FillPositionAsync(string position) => await PositionInput.FillWithDelayAsync(position);
    public async Task FillExperienceAsync(string years) => await ExperienceInput.FillWithDelayAsync(years);
    public async Task FillBioAsync(string bio) => await BioTextarea.FillWithDelayAsync(bio);
    public async Task FillSkillsAsync(string skills) => await SkillsInput.FillWithDelayAsync(skills);
    ...
    public async Task SetSalaryAsync(string salary) => await SalaryRange.FillWithDelayAsync(salary);
    public async Task SelectAvailabilityAsync(string availability) => await AvailabilitySelect.SelectWithDelayAsync(availability);
    ...
    public async Task CheckTermsAsync() => await TermsCheckbox.CheckWithDelayAsync();
    public async Task CheckNewsletterAsync() => await NewsletterCheckbox.CheckWithDelayAsync();

    // ============ Métodos de Submit y Mensaje ============
    public async Task SubmitFormAsync() => await SubmitButton.ClickWithDelayAsync();
